...
// keep track of how many unknown failures occur in a row, so the program can be auto-crashed
...
module.exports = function ({startTime}) {
  ...
  let q = async.priorityQueue(function ({id = xbeeAPI.nextFrameId(), data}, callback) {
    ...
    console.log(`Sending frame ${id} to prop ${prop}. Attempt #${data.attempts}`);
  ...
  q.drain = function () {
    ...
    Object.keys(attemptedFrames).forEach(id => {
      ...
      if (frame.deliveryStatus) {
        ...
        if (frame.deliveryStatus !== 'Unknown') {
          consecutiveUnknowns = 0;
        } else {
          consecutiveUnknowns++;
          if (consecutiveUnknowns > 10) throw new Error('Crashing program due to likely XBee connection issue.');
        }
      } else {
